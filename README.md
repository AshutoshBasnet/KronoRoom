...
Version 2.0 brings a streamlined, modern UI design, real-time cinema-style seat reservation, live occupancy telemetry, sub-second socket synchronization, role-based booking policies, and automated lifecycle background crons.

---

## 🚀 What's New in Version 2.0

- 🎨 **Modernized Glassmorphism UI:** Replaced legacy matrix/vision themes with a sleek, high-contrast London Met university aesthetic using Tailwind CSS v4, Lucide icons, and Framer Motion animations.
- 💺 **Interactive Cinema-Style Seat Map:** Visual seat selection mapped to exact room capacities (50 workstations, 80 seminar seats, 100 auditorium seats) with instant multi-seat picking (`A1, A2, A3`).
...
- 🤖 **Automated Auto-Release Daemon:** Background cron worker that monitors overdue bookings and releases unclaimed seats automatically every 5 minutes.
...
| Layer | Technologies |
| :--- | :--- |
| **Frontend** | React 19, Vite, Tailwind CSS v4, Framer Motion, Lucide React, Axios, Socket.io Client, React Router v7, date-fns |
| **Backend** | Node.js (ES Modules), Express.js, Socket.io, Mongoose (MongoDB Atlas), JSON Web Tokens (JWT), Bcrypt.js, Node-Cron |
| **Tooling & Architecture** | RESTful APIs, WebSockets, Role-Based Access Control (RBAC), Oxlint |
...
- Dynamic grid generation for each room type:
  - 🟢 **Available (Green):** Open for immediate reservation.
  - 🟡 **Upcoming Reservation (Yellow):** Booked for an upcoming session.
  - 🔴 **In-Session (Red):** Actively occupied with live countdown.
  - 🟣 **Selected (Indigo):** Current user's active seat selection.

### 2. Role-Based Access Control (RBAC)
- **👨‍🎓 Students:** Maximum 2-hour sessions, advance reservation window up to 3 days.
- **👨‍🏫 Faculty:** Extended 6-hour lecture/lab allocations, advance booking up to 30 days.
- **🛡️ Administrators:** Full campus analytics, room CRUD management, user controls, and force-cancellation overrides.

### 3. London Met Academic Module Catalog
Pre-configured with official modules:
- `CC5051NI` — Databases
...
```
*(If no `MONGO_URI` is supplied or remote DB is unreachable, KronoRoom automatically launches an in-memory database).*
